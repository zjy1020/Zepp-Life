...
- 包名 `com.zepplife.steps`
- **应用内一键更新**：在「日志」页点「检查更新」，发现新版本后旁边会出现「立即更新」，
  点它即会在应用内下载并唤起系统安装界面（首次需允许「安装未知应用」）
- 签名使用项目自带的**固定密钥**（存于 GitHub Secrets，不入库），
  因此各版本签名一致，升级可直接覆盖安装、不会丢数据

> **从 v1.0.7 之前升级到 v1.0.7 需要先卸载一次。**
> 早期版本的构建没有配置 keystore，CI 每次都在全新虚拟机上现场生成调试密钥，
> 导致 v1.0.4 / v1.0.5 / v1.0.6 的签名互不相同，Android 拒绝覆盖安装。
> v1.0.7 起改用固定密钥，此后不再需要卸载。
